...
x = model.laplacian_c_inverse_dct(lapf).squeeze()
...
im = a.flatten()[0].imshow(x.cpu().numpy().T, cmap='bwr', origin='lower', vmin=um, vmax=uM,extent=[0,param['Lx'],0,param['Ly']], aspect=param['Lx']/param['Ly'])
...
im = a.flatten()[1].imshow(err.T, cmap='bwr', origin='lower',extent=[0,param['Lx'],0,param['Ly']], aspect=param['Lx']/param['Ly'])
f.colorbar(im, ax=a.flatten()[1],fraction=0.046,location='bottom')
a.flatten()[1].set_title('f-f_{exact}')
